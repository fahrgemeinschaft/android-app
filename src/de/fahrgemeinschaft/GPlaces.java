...
public class GPlaces {
	...
	static public class AutocompleteLoader extends AsyncTaskLoader<Cursor> {
		...
		private MatrixCursor places;
		private String text;

		public AutocompleteLoader(Context context, String text) {
			super(context);
			this.text = text;
		}
		...
		public Cursor loadInBackground() {
			...
			try {
				...
				places = new MatrixCursor(new String[] { "_id", "name",
						"address", "geohash" });
				for (int i = 0; i < predsJsonArray.length(); i++) {
					JSONObject json = predsJsonArray.getJSONObject(i);
					String[] place = json.get("description").toString()
							.split(", ");
					Log.d(TAG, json.get("description").toString());
					if (place.length > 1) {
						places.addRow(new String[] { "" + i, place[0],
